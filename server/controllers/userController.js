...
// 		//creating line items(details to list) for stripe
// 		const line_items = [
// 			{
// 				price_data: {
// 					currency,
...
export const purchaseCourse = async (req, res) => {
  try {
    ...
    // Correct amount calculation and ObjectId
...
export const addUserRating = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    if (!user || !user.enrolledCourses.includes(mongoose.Types.ObjectId(courseId))) {
    ...
    if (existingRatingIndex > -1) {
      ...
      course.courseRatings.push({ userId: mongoose.Types.ObjectId(userId), rating });
    }
